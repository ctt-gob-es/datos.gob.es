...
function get_json_array_value($field_name, $json_data)
{
  $field = get_json_value($field_name, $json_data);
  $field_string = "";
  if($field != ''){
    foreach ($field as $field_value) {
        $field_string .= get_json_label($field_name, $field_value).", ";
    }
    $field_string = substr($field_string, 0, -2);
  }
...
{
  $path = drupal_get_path('module', 'dge_widget') . '/json/' . $file_name;
...
function get_json_label($field_name, $field_value)
{
  $json_label = "";
    ...
    if($name == $field_name){
      ...
      foreach ($choices as $choice) {
        ...
        if($value == $field_value){
          $json_label = get_json_key_value('label', 'es', $choice);
        }
      }
    }
  }
