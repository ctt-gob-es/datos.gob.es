...
class DGERDFHarvester(DCATRDFHarvester):
    ...
    def _get_content_and_type(self, url, harvest_job, page=1, content_type=None):
        ...
        try:
            ...
            if cl and int(cl) > self.MAX_FILE_SIZE:
                log.debug("se mete en el segundo if")
            ...
            for chunk in r.iter_content(chunk_size=self.CHUNK_SIZE):
                ...
                if length >= self.MAX_FILE_SIZE:
                    ...
                    self._save_gather_error(errormsg, harvest_job)
                    return None, None
            # Prueba para Comunidad Madrid
            # file_test = urllib2.urlopen(url)
    ...
    def _get_guid(self, dataset_dict, source_url=None):
        ...
        if dataset_dict.get(dhc.DS_URI):
            uri = dataset_dict.get(dhc.DS_URI).replace('https://', 'http://')
            if dataset_dict.get(dhc.DS_PUBLISHER_ID_MINHAP):
                return dataset_dict.get(dhc.DS_PUBLISHER_ID_MINHAP) + "-" + uri
            else:
                return uri

        if dataset_dict.get(dhc.DS_IDENTIFIER):
            uri = dataset_dict.get(dhc.DS_IDENTIFIER).replace('https://', 'http://')
            if dataset_dict.get(dhc.DS_PUBLISHER_ID_MINHAP):
                return dataset_dict.get(dhc.DS_PUBLISHER_ID_MINHAP) + "-" + uri
            else:
                return uri
        ...
        if dataset_dict.get(dhc.DS_NAME):
            ...
            if source_url:
                guid = source_url.replace('https://', 'http://').rstrip('/') + '/' + guid
    ...
    def gather_stage(self, harvest_job):
        ...
        while url is not None:
            for harvester in p.PluginImplementations(IDCATRDFHarvester):
                ...
                if not url:
                    log.debug('%s End method 1. Returns: False' % (method_log_prefix))
                    return False

            rdf_format = None
            default_catalog_language = None
            if harvest_job.source.config:
                rdf_format = self.config.get(dhc.HS_PROP_RDF_FORMAT, "rdf_format")
            ...
            if not content:
                log.debug('%s End method 2. Returns: False' % (method_log_prefix))
                return False
            ...
            try:
                ...
                self._save_gather_error(errormsg, harvest_job)
                log.debug('%s End method 3. Returns: False' % (method_log_prefix))
                return False
            ...
            try:
                ...
                self._save_gather_error(errormsg, harvest_job)
                log.debug('%s' % (content))
                log.debug('%s End method 4. Returns: False' % (method_log_prefix))
                return False
                ...
                if url:
                    errormsg = dhc.CATALOG_PARSER_ERROR_URL % (url, e)
                self._save_gather_error(errormsg, harvest_job)
                log.debug('%s End method 5. Returns: False' % (method_log_prefix))
                return False
            ...
            try:
                for catalog in parser.catalogs():
                    ...
                    if catalog_errors and len(catalog_errors) > 0:
                        ...
                        log.debug('%s End method 6. Returns: False' % (method_log_prefix))
                        return False
                    else:
                        ...
                        try:
                            ...
                            for dataset in parser.datasets(dict):
                                ...
                                if not dataset.get(dhc.DS_NAME) \
                                    ...
                                    dataset[dhc.DS_NAME] = self._gen_new_name(
                                        dataset.get(dhc.DS_PUBLISHER_ID_MINHAP, '') + '-' + dataset.get(dhc.DS_TITLE,
                                                                                                        ''))
                                ...
                                if not guid:
                                    log.error('%s Could not get a unique identifier for dataset: %s' % (
                                        method_log_prefix, dataset))
                                    continue
                                ...
                                if errors and len(errors) > 0:
                                    ...
                                    for error in errors:
                                        num = num + 1
                                        if (num <= MAX_NUM):
                                            log.info("%s Saving objectError %s for guid %s" % (
                                                method_log_prefix, error, guid))
                                ...
                                if warnings and len(warnings) > 0:
                                    ...
                                    for warn in warnings:
                                        num = num + 1
                                        if (num <= MAX_NUM):
                                            log.info("%s Saving warning in objectError %r for guid %r" % (
                                                method_log_prefix, warn, guid))
                                            warnmessage = dhc.DATASET_VALIDATION_WARNING % (warn)
                                            self._save_object_error(warnmessage, obj, 'Gather')

                            # Summary
                            summarymsg = dhc.LOG_SUMMARY % (
                            ...
                            summarymsg = dhc.LOG_SUMMARY % (
                                ...
                                total_warnings)
                            log.info("%s %s" % (method_log_prefix, summarymsg))
                            log.debug('%s End method 7. Returns: False' % (method_log_prefix))
                            return False
            except RDFParserException, e:
                ...
                log.info("%s %s" % (method_log_prefix, summarymsg))
                log.debug('%s End method 8. Returns: False' % (method_log_prefix))
                return False
            url = parser.next_page()
            log.debug('%s Getting next page URL: %s' % (method_log_prefix, url))
        ...
        log.debug('%s' % (content))
        log.debug('%s End method 9. Returns: %s' % (method_log_prefix, object_ids))
        return object_ids
    ...
    def import_stage(self, harvest_object):
        ...
        try:
            ...
            if harvest_object.content is None:
                ...
                log.info("%s Saving objectError %s for harvest_object_guid %s" % (
                    method_log_prefix, error, harvest_object_guid))
            ...
            if existing_dataset:
                ...
                try:
                    ...
                    log.debug("%s Saving objectError %s for harvest_object_guid %s" % (
                        method_log_prefix, error, harvest_object_guid))
            ...
            errormsg = '%s' % e
            try:
                errormsg = "Exception in harvest_object %s (%s) %s: %s" % (
                    harvest_object_guid, harvest_object_id, type(e).__name__, e)
