...
PUBLISHER_TYPES = ['manual_loading_publishers', 'harvester_publishers']

DRUPAL_CONTENT_TYPES = {'app': {'color': "#76BEDF" , 'bullet': "triangleDown", 
                                'bulletSize': 10, "position": "left", 
                                "offset": 0, "title": 'apps'},
                        'initiative': {'color': "#FF6600" , 'bullet': "square", 
                                       'bulletSize': 8, "position": "right", 
                        ...
                        'success': {'color': "#A1C51B" , 'bullet': "triangleUp", 
                                    ...
                                    "offset": 50, "title": 'successes'}}

EXCLUDED_DRUPAL_CONTENT_TYPES = ['request']

SECTIONS = {'aplicaciones': {'color': "#76BEDF" , 'bullet': "triangleDown", 
                             'bulletSize': 10, },
            'iniciativas': {'color': "#FF6600" , 'bullet': "square", 
                            'bulletSize': 8}, 
            'peticiones-datos': {'color': "#FCD202" , 'bullet': "diamond", 
                                 'bulletSize': 11}, 
            'casos-exito': {'color': "#A1C51B" , 'bullet': "triangleUp", 
                            'bulletSize': 10},
            'dashboard': {'color': "#0000CC" , 'bullet': "triangleRight", 
                          'bulletSize': 10},
            'catalogo': {'color': "#D1655D" , 'bullet': "round", 
                         'bulletSize': 10},
            'documentacion': {'color': "#CD0D74" , 'bullet': "triangleLeft", 
                              'bulletSize': 10}}

PUBLISHER_TYPES = {'harvester_publishers': {'color': "#C64A1A" , 'bullet': "bubble", 
                             'bulletSize': 12, },
                   'manual_loading_publishers': {'color': "#D0D0D0" , 'bullet': "bubble", 
                            'bulletSize': 12}}
                    ...
                    if year:
                        if id_organization:
                            if item.get('org') and item.get('org') == id_organization:
                                month_data[item.get('year')] = {'year': item.get('year'), 'content_comments': item.get('content_comments') if item.get('content_comments') else 0, 'dataset_comments': item.get('dataset_comments') if item.get('dataset_comments') else 0}
                        else:
                            month_data[item.get('year')] = {'year': item.get('year'), 'content_comments': item.get('content_comments') if item.get('content_comments') else 0, 'dataset_comments': item.get('dataset_comments') if item.get('dataset_comments') else 0}
...
def dge_dashboard_data_num_datasets_by_administration_level():
    ...
    data_date = None
    try:
        url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.datasets_administration_level.url_data', None))
        if url:
            response = urllib.urlopen(url)
            if response:
                data = json.loads(response.read())
        ...
        log.error('Exception in dge_dashboard_data_num_datasets_by_administration_level: %s', e)
...
def dge_dashboard_data_distribution_format_by_administration_level():
    ...
    total = 0
    try:
        url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.distribution_format_administration_level.url_data', None))
        if url:
            response = urllib.urlopen(url)
            if response:
                data = json.loads(response.read())
...
def dge_dashboard_data_num_drupal_contents():
    ...
    result_graphs = None
    try:
        url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.drupal_contents_month_year.url_data', None))
        ...
        for name in graph_names:
            if name:
                if name == 'chartVisitsDatosGobEsByMonth':
                    ...
                    visibility[name] = converters.asbool(
                                       config.get('ckanext.dge_dashboard.chart.distribution_format_administration_level.visible', False))    
    return visibility
...
def dge_dashboard_organization_data_num_datasets_by_month_year():
    ...
    try:
        ...
        if organization_id:
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.org.datasets_month_year.url_data', None))
            if url:
...
def dge_dashboard_organization_data_distribution_format():
    ...
    try:
        ...
        organization_id = organization.get('id', None) if organization else None
        if organization_id:
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.org.distribution_format.url_data', None))
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
                if response:
...
def dge_dashboard_organization_data_assigned_requests():
    ...
    try:
        ...
        organization_id = organization.get('id', None) if organization else None
        if organization_id:
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.org.assigned_requests.url_data', None))
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
                if response:
            ...
            if data:
                ...
                for item in data:
                    ...
                    if org and org == organization_id:
                        ...
                        if result_data is None:
                            result_data = []
                        result_data.append({'date': item.get('date',''), 'state': item.get('state', ''), 'value': item.get('value', 0)})
                        total = total + item.get('value', 0)
            if result_data:
                 return json.dumps(result_data), total, data_date, error_loading_data
    except Exception as e:
...
def dge_dashboard_organization_data_num_comments_by_month_year():
    ...
    try:
        ...
        organization_id = organization.get('id', None) if organization else None
        if organization_id:
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.org.comments_month_year.url_data', None))
            if url:
                error_loading_data = False
...
def dge_dashboard_organization_data_most_visited_datasets():
    ...
    try:
        ...
        organization_id = organization.get('id', None) if organization else None
        if organization_id:
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.org.most_visited_datasets.url_data', None))
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
                if response:
        ...
        error_loading_data = True
    return json.dumps(result_data), json.dumps(month_name_list), month_name_list, json.dumps(column_titles), error_loading_data
...
def dge_dashboard_administrator_data_num_datasets_by_administration_level():
    ...
    try:
        if _dge_dashboard_user_is_sysadmin():
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.adm.datasets_administration_level.url_data', None))
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
                if response:
...
def dge_dashboard_administrator_data_num_datasets_by_organization():
    ...
    try:
        if _dge_dashboard_user_is_sysadmin():
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.adm.datasets_month_year_org.url_data', None))
            data = None
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
...
def dge_dashboard_administrator_data_num_datasets_by_num_resources():
    ...
    try:
        if _dge_dashboard_user_is_sysadmin():
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.adm.datasets_month_year_num_res.url_data', None))
            data = None
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
...
def dge_dashboard_administrator_data_num_publishers_by_month_year():
    ...
    try:
        if _dge_dashboard_user_is_sysadmin():
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.adm.publishers_month_year.url_data', None))
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
                if response:
...
def dge_dashboard_administrator_data_num_publishers_by_administration_level():
    ...
    error_loading_data = False
    try:
        url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.adm.publishers_adm_level.url_data', None))
        if url:
            response = urllib.urlopen(url)
            if response:
                data = json.loads(response.read())
            if data:
                for item in data:
                    ...
                    if adm_level:
                        nitems[adm_level] = {'date': item.get('date',''), 'category': _dge_dashboard_get_translated_administration_level(adm_level)}
            ...
            for key in PUBLISHER_TYPES:
                result_graphs.append({"balloonText": "<b>[[title]]</b><br><span style='font-size:14px'>[[category]]: <b>[[value]]</b></span>",
                                      "type": "column",
                                      "title": _(key),
                                      "valueField": key,
                                      "fillAlphas": 1,
...
def dge_dashboard_administrator_data_assigned_requests():
    ...
    try:
        if _dge_dashboard_user_is_sysadmin():
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.adm.assigned_requests.url_data', None))
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
                if response:
            ...
            if data:
                ...
                for item in data:
                    if item.get('state', None):
                        if not data_date:
                            data_date = _dge_dashboard_convert_date(item.get('date', None))
                        result_data.append({'date': item.get('date', ''), 'state': _(item.get('state', '')), 'value': item.get('value', 0)})
                        total = total + item.get('value', 0)
                return json.dumps(result_data), total, data_date, error_loading_data
    except Exception as e:
...
def dge_dashboard_administrator_data_users_by_adm_level():
    ...
    try:
        if _dge_dashboard_user_is_sysadmin():
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.adm.users_adm_level.url_data', None))
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
                if response:
...
def dge_dashboard_administrator_published_drupal_contents():
    ...
    try:
        if _dge_dashboard_user_is_sysadmin():
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.adm.pusblished_drupal_contents.url_data', None))
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
                if response:
            ...
            if data:
                for item in data:
                    ...
                    if content_type:
                        ...
                        if not data_date:
                            data_date = _dge_dashboard_convert_date(item.get('date', None))
                        result_data.append({'content_type': _(content_type), 'num_contents': num_contents, 'date': data_date})
...
def dge_dashboard_administrator_data_num_comments_by_month_year():
    ...
    try:
        if _dge_dashboard_user_is_sysadmin():
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.adm.comments_month_year.url_data', None))
            if url:
                error_loading_data = False
...
def dge_dashboard_administrator_data_num_visits_by_section():
    ...
    try:
        if _dge_dashboard_user_is_sysadmin():
            url = _dge_dashboard_get_backend(config.get('ckanext.dge_dashboard.chart.adm.visits_month_year.url_data', None))
            if url:
                error_loading_data = False
                response = urllib.urlopen(url)
                if response:
